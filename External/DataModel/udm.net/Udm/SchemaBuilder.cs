...
namespace Unity.DataModel
{
    ...
    {
        public SchemaBuilder(IntPtr schema_manager_ptr, string typeName, UdmTypeId typeID, ulong typeVersion, TypeLayout typeLayout = default)
        {
            ...
            unsafe
            {
                ...
                UdmInterop.Instance.udm_schema_builder_set_as_managed(SchemaBuilderPtr, 1);
            }
        }

        public SchemaBuilder(IntPtr schema_manager_ptr, Schema baseSchema, string typeName, UdmTypeId typeID, ulong typeVersion, TypeLayout typeLayout = default)
        {
            if (String.IsNullOrEmpty(typeName))
                throw new ArgumentException("The type name cannot be null or an empty string.");
        ...
        internal SchemaBuilder SetInlineTextSerialization(bool inlineTextSerialization)
        {
            UdmInterop.Instance.udm_schema_builder_set_inline_text_serialization(SchemaBuilderPtr, inlineTextSerialization ? 1 : 0);
            return this;
        }

        internal SchemaBuilder SetAsFixedBuffer(bool is_fixed_buffer)
        {
            UdmInterop.Instance.udm_schema_builder_set_as_fixed_buffer(SchemaBuilderPtr, is_fixed_buffer ? 1 : 0);
            return this;
        }

        internal SchemaBuilder SetUnderlyingTypeId(UdmTypeId underlyingTypeID)
        {
            UdmInterop.Instance.udm_schema_builder_set_underlying_type_id(SchemaBuilderPtr, &underlyingTypeID);
            return this;
        }

        public SchemaBuilder AddFundamentalField<T>(string fieldName, int explicitOffset = -1, SerializeFrequency serializeFrequency = SerializeFrequency.Always, T defaultValue = default)
            where T : unmanaged
        {
        ...
        // TODO: DEFAULT VALUES
        public SchemaBuilder AddFundamentalVectorField<T>(string fieldName, VectorType[] vectorLevels, int explicitOffset = -1, SerializeFrequency serializeFrequency = SerializeFrequency.Always, ulong vectorRank = 1)
            where T : unmanaged
        {
            if (String.IsNullOrEmpty(fieldName))
                throw new ArgumentException("The name of fieldName can't be null or an empty string");
        ...
        internal SchemaBuilder AddReferenceField(string fieldName, int explicitOffset = -1, SerializeFrequency serializeFrequency = SerializeFrequency.Always, Reference defaultValue = default)
        {
            ...
            unsafe
            {
                ...
                var defaultValueAccessor = new ConstAccessor
                {
                    ...
                    Data = (IntPtr)(&reference),
                    References = (IntPtr)(&defaultValue)
                };
                AddSchemaField(fieldName, defaultValueAccessor, explicitOffset, serializeFrequency);
            }
            return this;
        ...
        public Schema BuildSchema(bool disposeBuilder = true)
        {
            ThrowIfInvalid();

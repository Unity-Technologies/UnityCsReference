...
namespace Unity.Timeline.Foundation.Widgets.Internals
{
    sealed class TickHandler
    {
        ...
        public void GetTicksAtLevel(int level, bool excludeTicksFromHigherLevels, List<float> list, bool allowNegativeTicks = true)
        {
            if (list == null)
                throw new ArgumentNullException(nameof(list));

            int clampedLevel = Mathf.Clamp(m_SmallestTickIndex + level, 0, m_TickModulos.Length - 1);
            int startTick = Mathf.FloorToInt(m_MinValue / m_TickModulos[clampedLevel]);
            int endTick = Mathf.CeilToInt(m_MaxValue / m_TickModulos[clampedLevel]);

            if (!allowNegativeTicks)
            {
                startTick = Mathf.Max(0, startTick);
                endTick = Mathf.Max(0, endTick);
            }

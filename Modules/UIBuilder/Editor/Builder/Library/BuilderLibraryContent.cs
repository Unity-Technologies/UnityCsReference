...
namespace Unity.UI.Builder
{
    static partial class BuilderLibraryContent
    {
        ...
        // Per-Type icon and item lookup filled by UpdateControlsTypeCache while the trees are generated,
        // so it refills with the trees on the next regeneration.
        [IgnoreForUAL0015("Per-Type library item cache refilled by UpdateControlsTypeCache on regeneration")]
        ...
        [AutoStaticsCleanupOnCodeReload]
        // Guard that skips regeneration when the project UXML paths are unchanged: a cleared hash never
        // matches the freshly computed one, so the next regeneration runs in full and stores it again.
        [IgnoreForUAL0015("Skip-regeneration hash; a cleared value forces a full rebuild that recomputes it")]
        ...
        [AutoStaticsCleanupOnCodeReload]
        // Diagnostic counter for tests that watch library regeneration; it starts at zero and is bumped
        // by each regeneration.
        [IgnoreForUAL0015("Diagnostic regeneration counter, incremented again by each regeneration")]
        internal static int libraryRegenerationTestCounter = 0;

        [AutoStaticsCleanupOnCodeReload]
        public static event Action OnLibraryContentUpdated;
        [AutoStaticsCleanupOnCodeReload]
        // A null tree is exactly what makes the next library regeneration call GenerateControlsItemsTree
        // again, so the standard controls come back from the built-in control types.
        [IgnoreForUAL0015("Standard controls tree regenerated by GenerateControlsItemsTree when null")]
        public static List<TreeViewItem> standardControlsTree { get; private set; }
        [AutoStaticsCleanupOnCodeReload]
        // Derived from standardControlsTree and rebuilt with it on the next regeneration.
        [IgnoreForUAL0015("Filtered view of the standard controls tree, rebuilt with it")]
        public static List<TreeViewItem> standardControlsTreeNoEditor { get; private set; }
        [AutoStaticsCleanupOnCodeReload]
        // Rebuilt from the project assets by GenerateProjectContentTrees during the next library
        // regeneration, which the asset processors trigger.
        [IgnoreForUAL0015("Project content tree rebuilt by the next library regeneration")]
        public static List<TreeViewItem> projectContentTree { get; private set; }
        [AutoStaticsCleanupOnCodeReload]
        // Rebuilt alongside projectContentTree during the next library regeneration.
        [IgnoreForUAL0015("Project content tree variant rebuilt by the next library regeneration")]

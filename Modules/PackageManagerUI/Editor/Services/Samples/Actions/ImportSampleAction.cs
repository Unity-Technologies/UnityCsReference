    ...
    internal class ImportSampleAction : SampleAction
    {
        ...
        public override string GetText(Sample sample, bool isInProgress)
        {
            if (sample.isImported)
                return L10n.Tr("Reimport", null);

            if (sample.previousImportPaths?.Count > 0)
                return L10n.Tr("Update", null);

            return L10n.Tr("Import", null);
        }

        public override string GetTooltip(Sample sample, bool isInProgress)
        {
            if (sample.isImported)
                return L10n.Tr("Click to reimport this sample into your project.", null);

            if (sample.previousImportPaths?.Count > 0)
                return L10n.Tr("Click to update this sample to the latest version.", null);

            return L10n.Tr("Click to import this sample into your project.", null);
        }
        ...
        protected override bool TriggerActionImplementation(Sample sample)
        {
            ...
            if (previousImports.Count > 0)
            {
                ...
                {
                    warningMessage = L10n.Tr("Different versions of the sample are already imported at", null) + "\n\n"
                        + previousImportPathsStringBuilder + "\n" + L10n.Tr("They will be deleted when you update.", null);
                }
                else
                {
                    if (sample.isImported)
                    {
                        warningMessage = L10n.Tr("The sample is already imported at", null) + "\n\n"
                            + previousImportPathsStringBuilder + "\n" + L10n.Tr("Importing again will override all changes you have made to it.", null);
                    }
                    else
                    {
                        warningMessage = L10n.Tr("A different version of the sample is already imported at", null) + "\n\n"
                            + previousImportPathsStringBuilder + "\n" + L10n.Tr("It will be deleted when you update.", null);
                    }
                }

                if (!m_Application.DisplayDialog("importPackageSample",
                        L10n.Tr("Importing package sample", null),
                        warningMessage + L10n.Tr(" Are you sure you want to continue?", null),

...
namespace UnityEditor.PackageManager.UI
{
    ...
    internal class RegistryInfoDraft
    {
        ...
        public int selectedScopeIndex
        {
            get
            {
                return m_SelectedScopeIndex;
            }
            set
            {
                m_SelectedScopeIndex = Math.Min(value, m_Scopes.Count - 1);
            }
        }
        ...
        public bool hasUnsavedChanges
        {
            get
            {
                ...
                return !comparer.Equals(original, new RegistryInfo(original.id, m_Name, m_Url, m_Scopes.ToArray(), original.isDefault));
            }
        }
        ...
        public bool isUrlOrScopesUpdated
        {
            get
            {
                return m_Modified && original != null &&
                    !new UpmRegistryClient.RegistryInfoComparer().Equals(original, new RegistryInfo(original.id, original.name, m_Url, m_Scopes.ToArray(), original.isDefault));
            }
        }
        ...
        public bool Validate()
        {
            ...
            Uri uriResult;
            if (!(Uri.TryCreate(m_Url, UriKind.Absolute, out uriResult) && (uriResult.Scheme == Uri.UriSchemeHttp || uriResult.Scheme == Uri.UriSchemeHttps)))

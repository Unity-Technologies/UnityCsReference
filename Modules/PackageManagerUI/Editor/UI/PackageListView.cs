...
namespace UnityEditor.PackageManager.UI.Internal
{
    internal class PackageListView : ListView, IPackageListView
    {
        ...
        public PackageListView()
        {
            ...
            itemHeight = PackageItem.k_MainItemHeight;
        ...
        {
            return new PackageItem(m_ResourceLoader, m_PageManager, m_SettingsProxy);
        }
        ...
        // Returns true if Refresh is triggered and false otherwise
        // In 2021.1 and earlier when itemsSource is assigned, refresh is called at the same time
        private bool UpdateItemsSource(List<VisualState> visualStates, bool alwaysReassignSource)
        {
            var newItemsSource = visualStates ?? new List<VisualState>();
            if (alwaysReassignSource || newItemsSource.Count != itemsSource?.Count)
            {
        ...
        {
            var refreshCalled = UpdateItemsSource(args.page.visualStates.ToList(), args.added.Any() || args.removed.Any());
            if (!refreshCalled)
            {

...
internal class LegacyFormatDropdownButton : BaseDropdownButton<GenericDropdownMenu>, IPackageToolBarButton
{
    ...
    public void Refresh(IPackageVersion version)
    {
        ...
        for (var i = 0; i < visibleItems.Count; ++i)
        {
            ...
            var itemEnabled = (item.state & ActionState.Disabled) == ActionState.None;
            dropdownMenu.AppendAction(item.text, itemEnabled, _ => item.action.TriggerAction(version), tooltip: item.tooltip);
        }
        menu = dropdownMenu;
    }

    private List<(PackageAction action, string text, string tooltip, ActionState state)> FindVisibleActions(IPackageVersion version)
    {
        var result = new List<(PackageAction action, string text, string tooltip, ActionState state)>();
        foreach (var action in m_Actions)
        {
            var state = action.GetActionState(version, out var text, out var actionTooltip);
            if ((state & ActionState.Visible) != ActionState.None)
                result.Add((action, text, actionTooltip, state));
        }
        return result;
    }

    private int FindMainActionIndex(IList<(PackageAction action, string text, string tooltip, ActionState state)> visibleItems, bool isRecommended)
    {
        for (var i = 0; i < visibleItems.Count; ++i)
            if (visibleItems[i].action.isRecommended == isRecommended && (visibleItems[i].state & ActionState.DisabledForItem) == ActionState.None)
                return i;
        for (var i = 0; i < visibleItems.Count; ++i)

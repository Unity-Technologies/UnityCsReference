...
namespace UnityEditor.PackageManager.UI
{
    internal class ScopedRegistriesSettings : VisualElement
    {
        ...
        private RegistryInfoDraft draft => PackageManagerProjectSettings.instance.registryInfoDraft;

        public ScopedRegistriesSettings()
        {
            var root = Resources.GetTemplate("ScopedRegistriesSettings.uxml");
            Add(root);
            ...
            UpmRegistryClient.instance.onRegistryOperationError += OnRegistryOperationError;
        ...
        private void AddRegistryClicked()
        {
            ...
            PackageManagerProjectSettings.instance.isUserAddingNewScopedRegistry = true;
        ...
        private bool AnyPackageInstalledFromRegistry(string registryName)
        {
            if (string.IsNullOrEmpty(registryName))
                return false;
            return PackageInfo.GetAll().Any(p => p.registry?.name == registryName);
        }
        ...
        private void RemoveRegistryClicked()
        {
            if (draft.original != null)
            {
                ...
                if (deleteRegistry)
                    UpmRegistryClient.instance.RemoveRegistry(draft.original.name);
            }
            else
            {
                if (!ShowUnsavedChangesDialog())
                ...
                PackageManagerProjectSettings.instance.isUserAddingNewScopedRegistry = false;
                RevertChanges();
            }
        }
        ...
        private void ApplyChanges()
        {
            ...
            if (draft.Validate())
            {
                ...
                if (draft.original != null)
                    UpmRegistryClient.instance.UpdateRegistry(draft.original.name, draft.name, draft.url, scopes);
                else
                    UpmRegistryClient.instance.AddRegistry(draft.name, draft.url, scopes);
            }
            else
            {
                RefreshErrorBox();
        ...
        private void RevertChanges()
        {
            ...
            else
            {
                ...
                var lastScopedRegistry = PackageManagerProjectSettings.instance.scopedRegistries.LastOrDefault();
                if (lastScopedRegistry != null)
                {
                    PackageManagerProjectSettings.instance.SelectRegistry(lastScopedRegistry.name);
                    PackageManagerProjectSettings.instance.isUserAddingNewScopedRegistry = false;
                }
            }
            UpdateRegistryList();
        ...
        private Label GetRegistryLabel(string registryName)
        {
            ...
            Label label;
            return m_RegistryLabels.TryGetValue(registryName, out label) ? label : null;
        }
        ...
        private void OnRegistryLabelClicked(string registryName)
        {
            ...
            PackageManagerProjectSettings.instance.SelectRegistry(registryName);
        ...
        private void OnRegistryOperationError(string name, Error error)
        {
        ...
        private void UpdateRegistryList()
        {
            ...
            foreach (var registryInfo in PackageManagerProjectSettings.instance.scopedRegistries)
            {
            ...
            var showAddNewScopedRegistryLabel = draft.original == null || PackageManagerProjectSettings.instance.isUserAddingNewScopedRegistry;
        ...
        {
            var emptyErrorMessage = string.IsNullOrEmpty(draft.errorMessage);
            UIUtils.SetElementDisplay(scopedRegistryErrorBox, !emptyErrorMessage);
            if (!emptyErrorMessage)
                scopedRegistryErrorBox.Q<Label>("errorMessage").text = draft.errorMessage;
        }
        ...
        private VisualElement scopedRegistriesInfoBox => cache.Get<VisualElement>("scopedRegistriesInfoBox");
        private VisualElement scopedRegistryErrorBox => cache.Get<VisualElement>("scopedRegistryErrorBox");

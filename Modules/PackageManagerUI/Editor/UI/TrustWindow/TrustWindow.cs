...
{
    internal enum TrustWindowType
    {
        Active,
        Passive
    }

    internal static class TrustWindow
    {
        private const int k_WindowWidthWithTechnicalName = 680;
        private const int k_WindowWidthNoTechnicalName = 580;
        private const string k_ActiveWindowType = "ActiveTrust";
        private const string k_PassiveWindowType = "PassiveTrust";
        ...
        internal static bool ShouldProceedAfterTrustCheck(string packagePath, IntPtr nativeAssetPackageInfo, int productId, string packageName, string packageVersion, int uploadId, bool isReimport)
        {
            ...
            return Show(viewData) == TrustWindowReturnValue.ProceedAnyway;
        }
        ...
        {
            public TrustWindowType windowType;
        ...
        public static TrustWindowReturnValue Show(ViewData data)
        {
            var container = ServicesContainer.instance;
            var content = new TrustWindowContent(container.Resolve<IResourceLoader>(), container.Resolve<IApplicationProxy>(), data);
            return ModalWindowContainer.ShowModal(content) ? content.returnValue : TrustWindowReturnValue.Error;
        }
        ...
        public static ViewData CreateViewData(IUpmCache upmCache, IEnumerable<PackageInfo> packages, OperationType operationType, string shortUnityVersion, TrustPolicyLevel trustPolicyLevel)
        {
            ...
            return new ViewData
            {
                windowType = TrustWindowType.Active,
        ...
        public static ViewData CreateViewData(IUpmCache upmCache, IEnumerable<IPackage> changedPackages, TrustPolicyLevel trustPolicyLevel, string shortUnityVersion)
        {
            var invalidSignatureRows = new List<RowData>();
            var missingSignatureRows = new List<RowData>();
            var limitedTrustRows = new List<RowData>();

            var limitedTrustIsBlocked = TrustAndSignatureHelper.IsBlocked(trustPolicyLevel, TrustAndSignature.LimitedTrust);

            foreach (var package in changedPackages)
            {
                var installedVersion = package.versions.installed;
                var info = upmCache.GetInstalledPackageInfo(package.name);
                if (info == null || installedVersion == null)
                    continue;

                switch (installedVersion.trustAndSignature)
                {
                    case TrustAndSignature.UntrustedInvalidSignature:
                        invalidSignatureRows.Add(new RowData(info));
                        break;
            ...
            if (invalidSignatureRows.Count > 0)
            {
                var isInvalidBlocked = TrustAndSignatureHelper.IsBlocked(trustPolicyLevel, TrustAndSignature.UntrustedInvalidSignature);
                var headerText = SelectByCount(invalidSignatureRows.Count,
                    L10n.Tr("This package has an invalid signature which can indicate unsafe or malicious content.", null),
                    L10n.Tr("{0} packages have invalid signatures which can indicate unsafe or malicious content.", null));
                sections.Add(new SectionData
                {
                ...
                windowTitle = L10n.Tr("Invalid Signature", null);
                helpBoxText = isInvalidBlocked
                    ? SelectByCount(invalidSignatureRows.Count,
                        L10n.Tr("This package has an invalid signature which can indicate unsafe or malicious content. This condition doesn't meet your security configuration. Remove this package to reduce risk to your project.", null),
                        L10n.Tr("These packages have an invalid signature which can indicate unsafe or malicious content. This condition doesn't meet your security configuration. Remove these packages to reduce risk to your project.", null))
                    : SelectByCount(invalidSignatureRows.Count,
                        L10n.Tr("This package has an invalid signature which can indicate unsafe or malicious content. Remove this package to reduce risk to your project.", null),
                        L10n.Tr("These packages have an invalid signature which can indicate unsafe or malicious content. Remove these packages to reduce risk to your project.", null));
                messageType = HelpBoxMessageType.Error;
                docUrl = $"https://docs.unity3d.com/{shortUnityVersion}/Documentation/Manual/upm-errors.html#pkg-invalid-sig";
            }
            ...
            if (missingSignatureRows.Count > 0)
            {
                var isMissingBlocked = TrustAndSignatureHelper.IsBlocked(trustPolicyLevel, TrustAndSignature.UntrustedNoSignature);
                var headerText = SelectByCount(missingSignatureRows.Count,
                    L10n.Tr("This package is missing a signature.", null),
                    L10n.Tr("{0} packages are missing a signature.", null));
                sections.Add(new SectionData
                ...
                {
                    windowTitle = L10n.Tr("Missing Signature", null);
                    helpBoxText = isMissingBlocked
                        ? SelectByCount(missingSignatureRows.Count,
                            L10n.Tr("Unity can't verify this package because it doesn't have a signature, which doesn't meet your security configuration. Use signed packages to reduce risk to your project.", null),
                            L10n.Tr("Unity can't verify these packages because they don't have a signature, which doesn't meet your security configuration. Use signed packages to reduce risk to your project.", null))
                        : SelectByCount(missingSignatureRows.Count,
                            L10n.Tr("Unity can't verify this package because it doesn't have a signature. Use signed packages to reduce risk to your project.", null),
                            L10n.Tr("Unity can't verify these packages because they don't have a signature. Use signed packages to reduce risk to your project.", null));
                    messageType = HelpBoxMessageType.Warning;
                    docUrl = $"https://docs.unity3d.com/{shortUnityVersion}/Documentation/Manual/upm-signature.html";
                }
            ...
            if (limitedTrustRows.Count > 0)
            {
                ...
                {
                    windowTitle = L10n.Tr("Unofficial Unity Source", null);
                    helpBoxText = SelectByCount(limitedTrustRows.Count,
                        L10n.Tr("This package is signed, but its publisher isn't verified by Unity. Ensure you understand where this package came from.", null),
                        L10n.Tr("These packages are signed, but their publishers aren't verified by Unity. Ensure you understand where these packages came from.", null));
                    messageType = HelpBoxMessageType.Warning;
                    docUrl = $"https://docs.unity3d.com/{shortUnityVersion}/Documentation/Manual/upm-signature.html";
            ...
            return new ViewData
            {
                windowType = TrustWindowType.Passive,
                windowTitle = windowTitle,
                helpBoxText = helpBoxText,
                helpBoxMessageType = messageType,
                actionLabel = null,
                ...
                analyticsData = new TrustAnalyticsData
                {
                    windowType = k_PassiveWindowType,
                    invalidSignaturePackageIds = ConvertRowDataToPackageIds(invalidSignatureRows),
                    missingSignaturePackageIds = ConvertRowDataToPackageIds(missingSignatureRows),
                    limitedTrustPackageIds = ConvertRowDataToPackageIds(limitedTrustRows)
                }
        ...
        internal class TrustWindowContent : ModalContent
        {
            public TrustWindowReturnValue returnValue { get; private set; } = TrustWindowReturnValue.Cancel;
            ...
            internal TrustWindowContent(IResourceLoader resourceLoader, IApplicationProxy application, ViewData data)
            {
                var root = resourceLoader.GetTemplate("TrustWindow.uxml");
                cache = new VisualElementCache(root);

                styleSheets.Add(resourceLoader.packageManagerCommonStyleSheet);
                styleSheets.Add(resourceLoader.trustWindowStyleSheet);
                Add(root);
                ...
                if (data.windowType == TrustWindowType.Passive)
                    BuildPassiveButtons();
                else
                    BuildActiveButtons(data);
            }

            private void BuildPassiveButtons()
            {
                var openPackageManagerButton = new Button { name = "openPackageManagerButton", text = L10n.Tr("Open Package Manager", null) };
                openPackageManagerButton.clicked += () =>
                {
                    returnValue = TrustWindowReturnValue.OpenPackageManager;
                    m_AnalyticsData.action = returnValue.ToString();
                    PackageManagerTrustWindowAnalytics.SendEvent(m_AnalyticsData);
                    container?.Close();
                };
                buttonsContainer.Add(openPackageManagerButton);

                var closeButton = new Button { name = "closeButton", text = L10n.Tr("Close", null) };
                closeButton.clicked += () =>
                {
                    returnValue = TrustWindowReturnValue.Cancel;
                    m_AnalyticsData.action = returnValue.ToString();
                    PackageManagerTrustWindowAnalytics.SendEvent(m_AnalyticsData);
                    container?.Close();
                };
            ...
            private void BuildActiveButtons(ViewData data)
            {
                if (!data.blockedBySecuritySettings)
                {
                    var proceedAnywayButton = new Button { name = "proceedAnywayButton", text = string.Format(L10n.Tr("{0} Anyway", null), data.actionLabel) };
                    proceedAnywayButton.clicked += () =>
                    {
                        returnValue = TrustWindowReturnValue.ProceedAnyway;
                        m_AnalyticsData.action = returnValue.ToString();
                        PackageManagerTrustWindowAnalytics.SendEvent(m_AnalyticsData);
                        container.Close();
                    };

...
namespace UnityEditor.PackageManager.UI.Internal
{
    internal class InProgressDropdown : DropdownContent
    {
        ...
        public override Vector2 windowSize => new(k_Width, m_Height);
        ...
        {
            var packagesInProgress = new List<string>(m_PackageDatabase.allPackages.Filter(p =>
                p.progress == progress && (tag == PackageTag.None || p.versions.primary.HasTag(tag))).SelectAsEnumerable(p => p.uniqueId));

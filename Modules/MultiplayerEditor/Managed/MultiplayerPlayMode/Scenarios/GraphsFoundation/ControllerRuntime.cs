...
{
    /// <summary>
    /// A ControllerRuntime is the run-time state of one configured controller: it contains an
    /// Execution Graph to coordinate the preparation, deployment and running of that controller's
    /// work. It also notifies all attached Execution Event Listeners of progress and results.
    /// Additionally, long-running events will be monitored via Monitoring tasks.
    /// The controller is usually a Virtual player that runs either in Editor, Clone Editors,
    /// Locally, Remotely, or on external devices.
    /// </summary>
    [Serializable]
    internal class ControllerRuntime : ISerializationCallbackReceiver
    {
        ...
        internal event Action<ControllerRuntime, InstanceStatusData> StatusRefreshed;

        // A scenario-scoped controller that declares no settings has no item to carry them, so the
        // controller's own name stands in and there is no id to address it by.
        internal string Name => m_ControllerItem != null ? m_ControllerItem.GetName() : Controller.name;
        internal GUID Id => m_ControllerItem != null ? m_ControllerItem.GetId() : default;
        ...
        internal static ControllerRuntime Create(
            ...
            ExecutionGraph executionGraph)
        {
            ...
            // For each controller, wire up an Execution Graph
            var runtime = new ControllerRuntime();
            runtime.m_ControllerItem = controllerItem;
            runtime.m_InstanceController = playModeController;
            runtime.m_ExecutionGraph = executionGraph;
            runtime.m_DecoratorsControllers = decorators;

            executionGraph.StatusRefreshed += runtime.OnGraphStatusRefreshed;
            runtime.OnGraphStatusRefreshed();
        ...
        internal static InstanceData[] GetAnalyticsDataArray(List<ControllerRuntime> instances)
        {
        ...
        // Returns the analytics InstanceData from this runtime
        ...
        internal async Task<bool> RunOrResumeAsync(ExecutionStage executionStage, CancellationToken cancellationToken)
        {
            ...
            // Await and signal completion after final stage

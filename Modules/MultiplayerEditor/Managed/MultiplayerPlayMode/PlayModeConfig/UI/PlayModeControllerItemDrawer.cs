...
[CustomPropertyDrawer(typeof(PlayModeControllerItem<,>))]
class PlayModeControllerItemDrawer : PropertyDrawer
{
    ...
    {
        var settingsProperty = property.FindPropertyRelative(IPlayModeControllerItem.k_SettingsPropertyPath);
    ...
    {
        var decoratorsProperty = property.FindPropertyRelative(IPlayModeControllerItem.k_DecoratorsPropertyPath);
        for (int i = 0; i < decoratorsProperty.arraySize; i++)
        {
            ...
            // A decorator whose settings are all hidden has nothing to show here, and would
            // otherwise contribute an empty row to the instance's settings.
            if (!decoratorProperty.hasVisibleChildren)
                continue;

...
interface IPlayModeControllerItem
{
    ...
    bool HasDecorator<TDecorator>() where TDecorator : PlayModeControllerDecorator;
    IDecoratorItem GetDecoratorItem(Type decoratorType);
    bool TryGetDecoratorItemIndex(Type decoratorType, out int index);
    IDecoratorItem GetDecoratorItem<TDecorator>() where TDecorator : PlayModeControllerDecorator;
    void GenerateMissingDecoratorsAndRemoveDuplicates(IEnumerable<Type> decoratorTypes);
    PlayModeController CreateController(OrchestratedScenario owner);

    IPlayModeControllerItem WithName(string name);
    IPlayModeControllerItem WithSettings<TSettings>(TSettings settings);
...
[Serializable]
struct PlayModeControllerItem<TController, TSettings> : IPlayModeControllerItem
    ...
    where TSettings : struct
{
    ...
    public PlayModeControllerItem(string name, TSettings settings)
    {
        ...
        {
            throw new InvalidOperationException( $"Failed to parse System.Guid '{systemGuid}' as Unity GUID for PlayModeControllerItem '{name}'. ");
        }

        m_Name = name;
        m_Settings = settings;

        GenerateMissingDecoratorsAndRemoveDuplicates(PlayModeControllerRegistry.GetDecoratorTypes(GetInstanceType()));
    }

    internal PlayModeControllerItem(GUID id, string name, TSettings settings, List<IDecoratorItem> decorators)
    {
        m_Id = id;
        m_Name = name;
        m_Settings = settings;
    ...
    public readonly bool IsInstanceType(Type type)
    {
        ...
        if (type.IsGenericTypeDefinition)
            return PlayModeController.IsSubclassOfGenericDefinition(typeof(TController), type);
    ...
    public readonly IDecoratorItem GetDecoratorItem<T>() where T : PlayModeControllerDecorator
        => GetDecoratorItem(typeof(T));

    public readonly bool HasDecorator(Type decoratorType)
    {
        if (!typeof(PlayModeControllerDecorator).IsAssignableFrom(decoratorType))
        {
            throw new ArgumentException($"Type {decoratorType.Name} must derive from PlayModeControllerDecorator.");
        }

        return TryGetDecoratorItemIndex(decoratorType, out _);
    }

    public readonly bool HasDecorator<T>() where T : PlayModeControllerDecorator
    ...
    public readonly bool TryGetDecoratorItemIndex(Type decoratorType, out int index)
    {
        index = -1;
    ...
    public void GenerateMissingDecoratorsAndRemoveDuplicates(IEnumerable<Type> decoratorTypes)
    {
        ...
        for (int i = m_Decorators.Count - 1; i >= 0; i--)
        {
            ...
            {
                Debug.LogWarning($"PlayModeControllerItem '{GetName()}' with id '{m_Id}' has multiple decorators of type '{decorator.GetDecoratorType().Name}'. These duplicate decorators will be removed.");
    ...
    public readonly PlayModeController CreateController(OrchestratedScenario owner)
    {
        return PlayModeController.CreateInstance<TController>(this, owner);
    }

    public readonly IPlayModeControllerItem WithName(string name)
    {
        var copy = this;
        copy.m_Name = name;
        return copy;
    ...
    public readonly IPlayModeControllerItem WithSettings<T>(T settings)
    {

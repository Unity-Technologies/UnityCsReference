...
namespace Unity.GraphToolkit.Editor
{
    ...
    /// </summary>
    [UnityRestricted]
    internal class TransitionView : AbstractTransition, IAnimatableView
    {
        /// <summary>
        /// The USS class name added to a <see cref="TransitionView"/>.
        /// </summary>
        public new const string ussClassName = "ge-transition";
        ...
        /// </summary>
        public const string transitionArrowPartName = "transition-arrow";

        /// <summary>
        /// The name used for the <see cref="ModelViewPart"/> of the transition control.
        /// </summary>
        public const string transitionControlPartName = "transition-control";

        /// <summary>
        /// The name for the "paste transitions as new" command.
        /// </summary>
        public const string pasteTransitionsAsNewCommandName = "Paste Transitions as New";
...
#pragma warning restore CS0649
        ...
        internal GradientFlowAnimator FlowAnimator { get; } = new();
        ...
        protected TransitionArrow TransitionArrow
        {
            get
            {
                ...
                {
                    if (TransitionControl != null) // Fetch the state-to-state transition arrow
                    {
                        m_TransitionArrow = TransitionControl.TransitionArrow;
                    }
                    else // Fetch the self transition arrow
                    {
        ...
        protected override void PostBuildUI()
        {
            ...
            if (TransitionModel.IsSelfTransition)
            {
                ...
                // Re-partition whenever the arrow's layout settles to the correct position.
                TransitionArrow?.RegisterCallback<GeometryChangedEvent>(OnTransitionArrowGeometryChanged);
            }
        }

        void OnTransitionArrowGeometryChanged(GeometryChangedEvent e)
        {
        ...
        public override Rect GetBoundingBox()
        {
            ...
            if (TransitionModel.IsSelfTransition && TransitionArrow != null)
                return TransitionArrow.ComputeBounds();

            return SizeElement.layout;
        }

        /// <summary>
        /// Applies a transition visual appearance to this view, or resets to the default appearance when <paramref name="appearance"/> is <see langword="null"/>.
        /// </summary>
        /// <param name="appearance">The appearance to apply, or <see langword="null"/> to reset to the default appearance.</param>
        public void SetAppearance(TransitionVisualData appearance)
        {
            appearance ??= new TransitionVisualData();
            ...
            float? widthOverride = appearance.WidthOverride == 0 ? null : appearance.WidthOverride;

            // Update wire appearance
            if (TransitionControl != null)
            {
                TransitionControl.IsDashedOverride = appearance.IsDashed;
                TransitionControl.LineWidthOverride = widthOverride;
                TransitionControl.OpacityMultiplier = appearance.Opacity;
                TransitionControl.ColorOverride = appearance.LineColor;
            }

            // Update arrow appearance
            if (TransitionArrow != null)
            {
                TransitionArrow.OpacityMultiplier = appearance.Opacity;
                TransitionArrow.OuterLineWidthOverride = widthOverride;
                TransitionArrow.OuterLineColorOverride = appearance.LineColor;
                TransitionArrow.FillColorOverride = appearance.FillColor;

                // IsDashed only reaches the arrow on a self transition
                if (TransitionArrow.IsSelfTransition)
                    TransitionArrow.IsDashedOverride = appearance.IsDashed;
            }

            TransitionControl?.MarkDirtyRepaint();
            TransitionArrow?.MarkDirtyRepaint();
        }

        /// <inheritdoc />
        public void BeginAnimating(float animationSpeed)
        {
            FlowAnimator.BeginAnimating(animationSpeed);
        }

        /// <inheritdoc />
        public void StopAnimating()
        {
            FlowAnimator.StopAnimating();
            TransitionControl?.MarkDirtyRepaint();
            TransitionArrow?.MarkDirtyRepaint();
        }
        ...
        {
            FlowAnimator.AnimationUpdate(deltaTime);
            TransitionControl?.MarkDirtyRepaint();
            TransitionArrow?.MarkDirtyRepaint();
        }
    }

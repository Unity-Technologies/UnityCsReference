...
using static Unity.U2D.Physics.PhysicsDestructorScripting2D;
using UnityEngine;
...
namespace Unity.U2D.Physics
{
    ...
    public readonly struct PhysicsDestructor
    {
        ...
        /// 
        /// If the resulting polygon fragments have more polygon vertices than can fit into a single <see cref="PolygonGeometry"/> then the fragment will be split into
        /// multiple polygon fragments. The maximum number of vertices a single polygon fragment can have is defined by <see cref="PhysicsConstants.MaxPolygonVertices"/>.
        /// 
        /// If even a single fragment point overlaps the target geometry then all results will be returned in <see cref="PhysicsDestructor.FragmentResult.brokenGeometry"/>.
        /// If none of the fragment points overlap the target geometry then all the results will be returned in <see cref="PhysicsDestructor.FragmentResult.unbrokenGeometry"/>.
        /// See <see cref="PhysicsDestructor.FragmentResult"/>.
        /// </summary>
        ...
        ///
        /// The target geometry is first clipped with the mask geometry using a <see cref="PhysicsComposer.Operation.NOT"/> operation. The resulting target geometry is returned in <see cref="PhysicsDestructor.FragmentResult.unbrokenGeometry"/>.
        /// The mask geometry is then clipped with the original target geometry using an <see cref="PhysicsComposer.Operation.AND"/> operation. If the clipped mask produces no geometry then no results are returned in <see cref="PhysicsDestructor.FragmentResult.brokenGeometry"/>.
        /// 
        /// The fragment points define areas where polygon fragments will be produced from the clipped masked geometry. The resulting polygon fragments are returned in <see cref="PhysicsDestructor.FragmentResult.brokenGeometry"/>.
        /// 
        /// If the resulting polygon fragments have more polygon vertices than can fit into a single <see cref="PolygonGeometry"/> then the fragment will be split into
        /// multiple polygon fragments. The maximum number of vertices a single polygon fragment can have is defined by <see cref="PhysicsConstants.MaxPolygonVertices"/>.
        /// See <see cref="PhysicsDestructor.FragmentResult"/>.
        ...
        /// 
        /// All valid target geometry will returned in either the <see cref="PhysicsDestructor.SliceResult.leftGeometry"/> or <see cref="PhysicsDestructor.SliceResult.rightGeometry"/> depending on its side of the line (sliced or not).
        ...
        /// See <see cref="PhysicsDestructor.SliceResult"/>.
        /// </summary>

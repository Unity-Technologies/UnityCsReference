...
namespace UnityEditorInternal
{
    internal class AnimationWindowHierarchyDataSource : TreeViewDataSource
    {
        ...
        private AnimationWindowHierarchyPropertyGroupNode AddPropertyGroupToHierarchy(AnimationWindowCurve[] curves, AnimationWindowHierarchyNode parentNode, SerializedObject so)
        {
            ...
            string groupDisplayName = AnimationWindowUtility.GetNicePropertyGroupDisplayName(curves[0].binding, so, animatableObjectType, out string fullPathForTooltip);

            AnimationWindowHierarchyPropertyGroupNode node = new AnimationWindowHierarchyPropertyGroupNode(animatableObjectType, 0, AnimationWindowUtility.GetPropertyGroupName(curves[0].propertyName), curves[0].path, parentNode, groupDisplayName);

            if (fullPathForTooltip != null)
                node.displayTooltip = fullPathForTooltip;
            ...
            foreach (AnimationWindowCurve curve in curves)
            {
                ...
                if (!string.IsNullOrEmpty(childNode.propertyName) && childNode.propertyName.IndexOf('/') >= 0)
                    childNode.displayName = AnimationWindowUtility.GetPropertyPartForDisplay(childNode.propertyName);
                else
        ...
        {
            string displayName = AnimationWindowUtility.GetNicePropertyDisplayName(curve.binding, so, curve.type, out string fullPathForTooltip);

            AnimationWindowHierarchyPropertyNode node = new AnimationWindowHierarchyPropertyNode(curve.type, 0, curve.propertyName, curve.path, parentNode, curve.binding, curve.isPPtrCurve, curve.isPhantom, curve.inheritanceState, displayName);
                ...
                if (hierarchyNode.curves != null)
                {
                    foreach (var curve in hierarchyNode.curves)
                    {
                        if (curve.isSerializeReferenceCurve && hierarchyNode.displayName.Contains(".Array.data["))
                        {
                            ...
                            hierarchyNode.displayName = AnimationWindowUtility.GetNicePropertyDisplayName(curve.binding, so, hierarchyNode.animatableObjectType, out string fullPathForTooltip);
                            hierarchyNode.displayTooltip = fullPathForTooltip;
                        }
                    }
                }
            }

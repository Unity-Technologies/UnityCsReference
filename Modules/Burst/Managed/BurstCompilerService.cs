...
namespace Unity.Burst.LowLevel
{
    internal static partial class BurstCompilerService
    {
        ...
        public static void Initialize(string folderRuntime, ExtractCompilerFlags extractCompilerFlags)
        {
            ...
            {
                Unity.Scripting.LowLevel.Debug.LogError($"Unable to initialize the burst JIT compiler. The folder `{folderRuntime}` does not exist");
                return;
            }
            ...
            {
                Unity.Scripting.LowLevel.Debug.LogError($"Unexpected error while trying to initialize the burst JIT compiler: {message}");
            }
        }
    }
}

...
namespace UnityEditor.Lighting.LightingSearch
{
    ...
    partial class LightmapSearchProvider : SearchProvider
    {
            ...
            : base(ProviderId, k_ProviderDisplayName)
        {
            ...
            onEnable = () =>
            {
                #pragma warning disable UAL0015 // OnBakeCompleted is static, so this captures no provider instance; the subscription is recreated because SearchService.Providers is a ScopedLazy<LazyInitStatics, CodeLoadedScope> that rebuilds this provider (and re-invokes this onEnable) whenever a SearchContext next activates it after reload

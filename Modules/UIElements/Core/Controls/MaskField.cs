...
namespace UnityEngine.UIElements
{
    ...
    public abstract class BaseMaskField<TChoice> : BasePopupField<TChoice, string>
    {
        ...
        static readonly string s_EverythingLabel = L10nUtility.GetTranslation("Everything");
        static readonly string s_NothingLabel = L10nUtility.GetTranslation("Nothing");
        ...
        void OnTextElementGeometryChanged(GeometryChangedEvent evt)
        {
        ...
        /// <summary>
        /// The list of masks for every specific choice to display in the popup menu.
        /// </summary>
        [CreateProperty]
        ...
        bool IsItemSelected(int maskOfItem)
        {
    ...
    /// <summary>
    /// The MaskField is a control that lets users select one or more options from a list of options.
    ...
    public class MaskField : BaseMaskField<int>
    {
        ...
        {
            var displayedValue = GetDisplayedValue(itemIndex);
        ...
        internal override string GetValueToDisplay()
        {
            var displayedValue = showMixedValue ? mixedValueString : GetDisplayedValue(rawValue);
            if (ShouldFormatSelectedValue())
                displayedValue = m_FormatSelectedValueCallback(displayedValue);
            return displayedValue;
        }

        bool ShouldFormatListItem(int itemIndex)
        {

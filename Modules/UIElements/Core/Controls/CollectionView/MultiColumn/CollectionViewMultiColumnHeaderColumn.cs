...
/// </summary>
class CollectionViewMultiColumnHeaderColumn : VisualElement
{
    ...
    /// </summary>
    public CollectionViewColumnMover mover { get; private set; }
    ...
    /// Default constructor.
    /// </summary>
    public CollectionViewMultiColumnHeaderColumn()
        : this(new Column()) {}
    ...
    public CollectionViewMultiColumnHeaderColumn(Column column)
    {
        this.column = column;
    ...
    {
        var header = this.GetFirstAncestorOfType<CollectionViewMultiColumnCollectionHeader>();
        var isFrozen = header != null && header.GetColumnFreezeState(column) != FreezeState.None;

        // Only add mover if column is not frozen
        if (!isFrozen)
        {
            this.AddManipulator(mover = new CollectionViewColumnMover());
    ...
    private void OnMoverChanged(CollectionViewColumnMover mv)
    {

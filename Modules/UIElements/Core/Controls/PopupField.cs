...
namespace UnityEngine.UIElements
{
    ...
    public class PopupField<T> : BasePopupField<T, T>
    {
        ...
        private void ChangeValueFromMenu(T menuItem)
        {
            ...
            if (prevIndex != m_Index && panel?.contextType == ContextType.Player && Insights.Enabled)
            {
                //Only log events which are directly attributed to user interaction.
                //Only the field's name is reported; choice values can contain PII.
                Insights.LogEvent(UI.InsightID.Dropdown_Selection, this);
            }
        }
    }
}

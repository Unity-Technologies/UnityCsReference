...
namespace UnityEngine.UIElements
{
    ...
    internal class VisualTreeStyleUpdaterTraversal : HierarchyTraversal
    {
        ...
        ComputedStyle ProcessMatchedRules(VisualElement element, List<SelectorMatchRecord> matchingSelectors)
        {
            ...
            // Re-resolve when a cached style's -unity-material was destroyed (e.g. an Addressables bundle
            // unloaded) so the reloaded material replaces the stale, now-invalid reference.
            if (!StyleCache.TryGetValue(matchingRulesHash, out var resolvedStyles) || HasStaleMaterial(ref resolvedStyles))
            {
        ...
        static bool HasStaleMaterial(ref ComputedStyle style)
        {
            // A destroyed material leaves a fake-null managed reference in the cached style; re-resolve so the
            // reloaded material is picked up instead of the default (white) fallback.
            var material = style.unityMaterial.material;
            return !ReferenceEquals(material, null) && material == null;
        }

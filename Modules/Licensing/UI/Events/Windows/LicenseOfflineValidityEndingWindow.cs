...
class LicenseOfflineValidityEndingWindow : TemplateLicenseEventWindow
{
    #pragma warning disable UAL0015 // this side effect does not outlive the current call (global trigger / lazily-loaded asset re-fetched on next access); a stale reference is harmlessly replaced
    public LicenseOfflineValidityEndingWindow() { }
    ...
    public static void ShowWindow(INativeApiWrapper nativeApiWrapper, ILicenseLogger licenseLogger, object notification)
    {
        ...
        s_Notification = notification as LicenseOfflineValidityEndingNotification;
        s_LicenseLogger = licenseLogger;

        TemplateLicenseEventWindow.ShowWindow<LicenseOfflineValidityEndingWindow>(LicenseTrStrings.OfflineValidityEndingWindowTitle, true);
    }

    public void CreateGUI()
    {
        s_Root = new LicenseOfflineValidityEndingWindowContents(
            (LicenseOfflineValidityEndingNotification)s_Notification,

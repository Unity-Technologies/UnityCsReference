        ...
        public void HandleKeyEvent(Event evt, IContextManager contextManager)
        {
            ...
            if (isKeyUpOrMouseUpEvent)
            {
                ...
                if (m_ActiveClutches.TryGetValue(keyCode, out clutchKeyValuePair))
                {
                    ...
                    var context = m_ActiveClutches[keyCode].Value;
            ...
            var isModifierKey = KeyCombination.k_KeyCodeToEventModifiers.TryGetValue(keyCode, out newModifier);
                ...
                if (evt.type == EventType.MouseDown)
                {
                    ...
                    else if (m_MouseActionEntries.Count == 1)
                    {
                        ...
                        if (m_ActiveMouseActionEntry.entry == null || m_ActiveMouseActionEntry.keyCode != keyCode)
                        {
                            ...
                            m_ActiveMouseActionEntry = (keyCode, m_MouseActionEntries[0], args.context);
                        }
            ...
            switch (entries.Count)
            {
                ...
                case 1:
                    ...
                    {
                        if (keyCode != m_KeyCombinationSequence[^1].keyCode)
                            break;
                        ...
                        switch (shortcutEntry.type)
                        {
                            ...
                            case ShortcutType.Clutch:
                                if (!m_ActiveClutches.ContainsKey(keyCode))
                                {
                                    ...
                                    m_ActiveClutches.Add(keyCode, keyValuePair);
                                    evt.Use();
                                }
                                break;
                        }

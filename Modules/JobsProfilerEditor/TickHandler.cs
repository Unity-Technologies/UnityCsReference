...
class TickHandler
{
    ...
    internal int tickLevels { get { return m_BiggestTick - m_SmallestTick + 1; } }
    ...
    internal List<float> GetTickModulosForFrameRate(float frameRate)
    {
        List<float> modulos;
    ...
    internal void GetTicksAtLevel(int level, bool excludeTicksFromHigherlevels, List<float> list)
    {
        if (list == null)

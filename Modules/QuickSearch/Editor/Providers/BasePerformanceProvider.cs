...
namespace UnityEditor.Search.Providers
{
    abstract class BasePerformanceProvider : SearchProvider
    {
        ...
        public static void PerformanceMetric(SearchColumn column)
        {
            column.binder = (SearchColumnEventArgs args, VisualElement ve) =>
            {
                ...
                if (args.item.provider is BasePerformanceProvider perfProvider)
                {
                    label.text = perfProvider.FormatColumnValue(args) ?? string.Empty;
                }
                else
                {
                    label.text = DefaultFormatColumnValue(args);
                }
            };
        }
    ...
    abstract class BasePerformanceProvider<TPerformanceData> : BasePerformanceProvider
    {
        ...
        {
            return m_QueryEngine.GetPropositions();
        }
        ...
        protected virtual QueryEngine<TPerformanceData> BuildQueryEngine()
        {
            ...
            qe.SetFilter(sampleAvgSelector, GetPerformanceAverageValue, supportedOperators)
                .AddOrUpdatePropositionData(category: "Performance", label: "Average", replacement: "avg>100", help: "Search by average value", color: QueryColors.filter);
            qe.SetFilter(sampleTotalSelector, GetPerformanceTotalValue, supportedOperators)
                .AddOrUpdatePropositionData(category: "Performance", label: "Total", replacement: "total>1000", help: "Search by total value", color: QueryColors.filter);

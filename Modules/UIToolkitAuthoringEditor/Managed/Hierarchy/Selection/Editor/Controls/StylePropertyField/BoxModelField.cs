...
namespace Unity.UIToolkit.Editor
{
    ...
    internal class BoxModelEditableLabel : StyleLengthField
    {
        ...
        public BoxModelEditableLabel() : base(null, false)
        {
        ...
        where TControl : BaseField<TValueType>
    {
        ...
        static readonly string CenterContentClassName = BoxModelClassName + "__center-content";
        static readonly string TextfieldClassName = BoxModelClassName + "__textfield";
        static readonly string ColorfieldClassName = BoxModelClassName + "__colorfield";
        static readonly string TitleClassName = BoxModelClassName + "__title";
        static readonly string CenterClassName = BoxModelClassName + "__center";
        ...
        public bool needsUnit
        {
            ...
            set
            {
                ...
                m_Title.AddToClassList(TitleClassName);
            ...
            Func<TControl> create)
        {
            ...
            foreach (var field in m_Fields)
            {
                if (typeof(TValueType) == typeof(StyleLength) || typeof(TValueType) == typeof(StyleFloat))
                    field.AddToClassList(TextfieldClassName);
                else if (typeof(TValueType) == typeof(StyleColor))
                    field.AddToClassList(ColorfieldClassName);
            }
            ...
            m_Center.AddToClassList(CenterClassName);
            m_Center.AddToClassList(InspectorCompositeStyleRowElementClassName);
        ...
        internal void UpdateUnitFromFields()
        {
            ...
            foreach (var field in m_Fields)
            {
                if (field is BoxModelEditableLabel editableLabel)
                    editableLabel.showUnit = unit == MixedUnitLabel;
            }
        }
        ...
        string GetUnitText()
        {
            ...
            if (boxType == BoxType.BorderColor)
            {
                ...
                unit = outerBoxModelUnit == innerBoxModelUnit ? outerBoxModelUnit : MixedUnitLabel;

                outerBoxModel.fields.ForEach(f => ((BoxModelEditableLabel)f).showUnit = unit == MixedUnitLabel);
                innerBoxModel.fields.ForEach(f => ((BoxModelEditableLabel)f).showUnit = unit == MixedUnitLabel);
            }
            else
            {

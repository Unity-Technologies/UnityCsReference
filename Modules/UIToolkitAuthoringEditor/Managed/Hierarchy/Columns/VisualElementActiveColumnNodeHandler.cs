...
internal static class VisualElementActiveColumnNodeHandler
{
    [HierarchyViewCellDescriptor(HierarchyWindowColumnActive.k_ColumnId, typeof(VisualElementNodeHandler))]
    internal static void CreateActiveColumnForAllStage(HierarchyViewCellDescriptor desc)
    {
    ...
    internal static void CreateActiveColumn(HierarchyViewCellDescriptor desc)
    {
        desc.BindCell = cell =>
        {
            var handler = cell.Handler as VisualElementNodeHandler;
            if (handler == null)
                return;

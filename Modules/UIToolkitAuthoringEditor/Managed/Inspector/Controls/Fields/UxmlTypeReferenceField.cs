...
{
    /// <summary>
    /// Field to a string property that allows selecting any type
    /// </summary>
    class UxmlTypeReferenceField : BaseField<string>
    {
        public const string UssClassName = "unity-uxml-type-reference-field";
        public const string TextUssClassName = UssClassName + "__text";
        public const string ButtonUssClassName = UssClassName + "__browse-type-button";

        readonly Type m_BaseType;
        readonly Label m_TextElement;

        public UxmlTypeReferenceField(Type baseType) : this(null, baseType) { }

        public UxmlTypeReferenceField(string label, Type baseType)
            : base(label, null)
        {
            m_BaseType = baseType;
    ...
    class TypeSearchProvider : SearchProvider
    {
        ...
        IEnumerable<SearchProposition> FetchPropositions(SearchContext context, SearchPropositionOptions options)
        {
            ...
            foreach (var asm in m_Assemblies)
            {
                ...
                yield return new SearchProposition("Assembly", assemblyName, $"{k_AssemblyToken}={assemblyName}",
        ...
        {
            var defaultColumns = new List<SearchColumn> { new SearchColumn("Name", "label") { width = 400 } };

...
sealed class UICanvasZoomManipulator : PointerManipulator
{
    const int MinScaleStart = 25;           // This is the minimum scale (25%), increment in small steps
    const int MiddleScaleStart = 150;       // At 150%, let's increment in larger steps
    const int MaxScaleStart = 500;          // At 500%, let's increment in even larger steps
    const int MaxScaleEnd = 10000;          // until we reach the max scale (10000%)
    const int LowScaleIncrement = 5;        // Low scale increment step (5%)
    const int MiddleScaleIncrement = 25;    // Middle scale increment step (25%)
    const int HighScaleIncrement = 250;     // High scale increment step (100%)
    ...
    static float CalculateNewZoom(float currentZoom, float wheelDelta, List<float> zoomValues)
    {
    ...
    {
        // Get current zoom scale and calculate new zoom scale
        var currentZoom = m_Canvas.ZoomScale;
        var newZoom = CalculateNewZoom(currentZoom, delta, zoomScaleValues);
        ...
        // Get current canvas offset
        var currentOffset = m_Canvas.Offset;

        // Calculate the point on the canvas under the zoom center
        // zoomCenter is in surface coordinates, so we need to account for the current offset
        var canvasPoint = zoomCenter - currentOffset;

        // After zooming, the same point on the canvas will be at: canvasPoint * zoomRatio
        // To keep that point under the zoom center, we need to adjust the offset:
        // zoomCenter = newOffset + canvasPoint * zoomRatio
        // newOffset = zoomCenter - canvasPoint * zoomRatio
        var newOffset = zoomCenter - canvasPoint * zoomRatio;

        using (m_Canvas.ManipulationScope())
        {
            // Apply the new zoom scale (this will update canvas size and document root scale)
            m_Canvas.ZoomScale = newZoom;

            // Apply the new offset to keep the zoom centered
            m_Canvas.Offset = newOffset;
        }
    }
}

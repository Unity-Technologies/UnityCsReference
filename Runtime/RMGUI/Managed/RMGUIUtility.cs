...
{
    internal class RMGUIUtility
    {
        private static Stack<IMGUIContainer> s_ContainerStack = new Stack<IMGUIContainer>();
        private static Dictionary<int, Panel> s_RMGUICache = new Dictionary<int, Panel>();
        ...
        {
            Panel panel;
            if (nativeEventPtr != IntPtr.Zero && s_RMGUICache.TryGetValue(instanceID, out panel))
            {
        ...
        {
            return s_RMGUICache.GetEnumerator();
        }
        ...
        {
            Panel panel;
            if (!s_RMGUICache.TryGetValue(instanceId, out panel))
            {
                ...
                s_RMGUICache.Add(instanceId, panel);
            }
            else
            {

...
namespace UnityEngine
{
    ...
    public sealed partial class Mesh : Object
    {
        ...
        private void SetUvsImpl<T>(int uvIndex, int dim, T[] uvs, int arrayStart, int arraySize, UnityEngine.Rendering.MeshUpdateFlags flags) where T : unmanaged
        {
            if (uvIndex < 0 || uvIndex > 7)
                throw new ArgumentOutOfRangeException(nameof(uvIndex), uvIndex, "The uv index is invalid. Must be in the range 0 to 7.");
            SetSizedArrayForChannel(GetUVChannel(uvIndex), VertexAttributeFormat.Float32, dim, MemoryMarshal.AsBytes(uvs.AsSpan()), NoAllocHelpers.SafeLength(uvs), arrayStart, arraySize, flags);
        }
        ...
        public static MeshDataArray AllocateWritableMeshData(List<Mesh> meshes)
        {
            if (meshes == null)
                throw new ArgumentNullException(nameof(meshes), "Mesh list is null");
            return new MeshDataArray(NoAllocHelpers.ExtractArrayFromList(meshes), meshes.Count, true, true);
        }
        ...
        private void SetTrianglesImpl<T>(int submesh, UnityEngine.Rendering.IndexFormat indicesFormat, T[] triangles, int trianglesArrayLength, int start, int length, bool calculateBounds, int baseVertex, int meshLod) where T : unmanaged
        {
            CheckIndicesArrayRange(trianglesArrayLength, start, length);
            SetIndicesImpl(submesh, MeshTopology.Triangles, indicesFormat, MemoryMarshal.AsBytes(triangles.AsSpan()), start, length, calculateBounds, baseVertex, meshLod);
        }
        ...
        public void SetIndices(int[] indices, int indicesStart, int indicesLength, MeshTopology topology, int submesh, int meshLod, bool calculateBounds = true, int baseVertex = 0)
        {
            if (CheckCanAccessSubmeshIndices(submesh))
            {
                ...
                SetIndicesImpl(submesh, topology, UnityEngine.Rendering.IndexFormat.UInt32, MemoryMarshal.AsBytes(indices.AsSpan()), indicesStart, indicesLength, calculateBounds, baseVertex, meshLod);
            }
        }
        ...
        public void SetIndices(ushort[] indices, int indicesStart, int indicesLength, MeshTopology topology, int submesh, int meshLod, bool calculateBounds = true, int baseVertex = 0)
        {
            ...
            {
                CheckIndicesArrayRange(NoAllocHelpers.SafeLength(indices), indicesStart, indicesLength);
                SetIndicesImpl(submesh, topology, UnityEngine.Rendering.IndexFormat.UInt16, MemoryMarshal.AsBytes(indices.AsSpan()), indicesStart, indicesLength, calculateBounds, baseVertex, meshLod);
            }
        }
        ...
        public void SetIndices(List<int> indices, int indicesStart, int indicesLength, MeshTopology topology, int submesh, int meshLod, bool calculateBounds = true, int baseVertex = 0)
        {
            if (CheckCanAccessSubmeshIndices(submesh))
            {
                ...
                CheckIndicesArrayRange(NoAllocHelpers.SafeLength(indices), indicesStart, indicesLength);
                SetIndicesImpl(submesh, topology, UnityEngine.Rendering.IndexFormat.UInt32, UnsafeUtility.GetByteSpanFromList(indices), indicesStart, indicesLength, calculateBounds, baseVertex, meshLod);
            }
        }
        ...
        public void SetIndices(List<ushort> indices, int indicesStart, int indicesLength, MeshTopology topology, int submesh, int meshLod, bool calculateBounds = true, int baseVertex = 0)
        {
            ...
            {
                CheckIndicesArrayRange(NoAllocHelpers.SafeLength(indices), indicesStart, indicesLength);
                SetIndicesImpl(submesh, topology, UnityEngine.Rendering.IndexFormat.UInt16, UnsafeUtility.GetByteSpanFromList(indices), indicesStart, indicesLength, calculateBounds, baseVertex, meshLod);
            }
        }
        ...
        {
            SetSubMeshes(NoAllocHelpers.ExtractArrayFromList<SubMeshDescriptor>(desc), start, count, flags);
        }
        ...
        {
            SetSubMeshes(NoAllocHelpers.ExtractArrayFromList<SubMeshDescriptor>(desc), 0, desc?.Count ?? 0, flags);
        }
    ...
    {
        private EntityId m_MeshInstanceID;
        private int m_SubMeshIndex;
        private Matrix4x4 m_Transform;
        private Vector4 m_LightmapScaleOffset;
        private Vector4 m_RealtimeLightmapScaleOffset;

        public Mesh mesh { get { return Mesh.FromInstanceID(m_MeshInstanceID); } set { m_MeshInstanceID = value != null ? value.GetEntityId() : EntityId.None; } }

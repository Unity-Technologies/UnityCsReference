...
static class UdmManager
{
    public static void CreateBasicUdmTypeData()
    {
        unsafe
        {
            ...
            // Fundamental types
            var uint8Schema = UdmInterop.Instance.udm_uint8_schema();
            var boolSchema = SchemaBuilder.BuildBasicSchemaWithUnderlyingType(schemaManager, uint8Schema->TypeId, "bool", UdmTypeId.HashString("System.Boolean"), 1);
            var byteSchema = SchemaBuilder.BuildBasicSchemaWithUnderlyingType(schemaManager, uint8Schema->TypeId, "byte", UdmTypeId.HashString("System.Byte"), 1);
            var charSchema = SchemaBuilder.BuildBasicSchemaWithUnderlyingType(schemaManager, uint8Schema->TypeId, "char", UdmTypeId.HashString("System.Char"), 1);
            ...
            // Unity types
            var unityHashSchema = SchemaBuilder.BuildBasicSchemaWithUnderlyingType(schemaManager, hashSchema->TypeId, "hash", UdmTypeId.HashString("UnityEngine.Hash128"), 1);
            var unityGuidSchema = SchemaBuilder.BuildBasicSchemaWithUnderlyingType(schemaManager, guidSchema->TypeId, "guid", UdmTypeId.HashString("UnityEngine.GUID"), 1);
            var entityIdSchema  = SchemaBuilder.BuildBasicSchemaWithUnderlyingType(schemaManager, referenceSchema->TypeId, "entityId", UdmTypeId.HashString("UnityEngine.EntityId"), 1);

            TypeTraits<UnityEngine.Hash128> .Set(new TypeTraitsData { TypeName = "hash", Schema = unityHashSchema});
            TypeTraits<UnityEngine.GUID>    .Set(new TypeTraitsData { TypeName = "guid", Schema = unityGuidSchema});

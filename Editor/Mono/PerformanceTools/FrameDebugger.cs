...
namespace UnityEditor
{
    internal class FrameDebuggerWindow : EditorWindow
    {
        ...
        private void DrawRenderTargetControls()
        {
            ...
            if (cur.rtLoadAction >= 0 || cur.rtStoreAction >= 0 || cur.rtMemoryless != 0 || cur.rtDepthLoadAction != 0 || cur.rtDepthStoreAction != 0)
            {
                ...
                if (cur.rtDepthLoadAction >= 0)
                    sl.Add(String.Format("Depth LoadAction: {0}", (RenderBufferLoadAction)cur.rtDepthLoadAction));
                if (cur.rtDepthStoreAction >= 0)
                    sl.Add(String.Format("Depth StoreAction: {0}", (RenderBufferStoreAction)cur.rtDepthStoreAction));

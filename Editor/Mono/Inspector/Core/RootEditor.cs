...
namespace UnityEditor
{
    ...
    internal static partial class RootEditorUtils
    {
        ...
        // Transient guard toggled around a single root-editor query; false is the correct resting value.
        [AutoStaticsCleanupOnCodeReload]
        private static bool s_SuppressRootEditor = false;

        // Holds handlers pointing at reloadable editor types, so it must not survive a reload; the
        // [OnCodeLoaded] Initialize() below rebuilds it from the attributes on every load.
        [AutoStaticsCleanupOnCodeReload]

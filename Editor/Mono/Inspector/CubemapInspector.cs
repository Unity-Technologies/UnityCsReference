        ...
        private void InitTexturesFromCubemap()
        {
            ...
            for (int i = 0; i < m_Images.Length; ++i)
            {
                ...
                else
                {
                    ...
                    if (GraphicsFormatUtility.IsCompressedTextureFormat(c.format) && SystemInfo.copyTextureSupport.HasFlag(CopyTextureSupport.DifferentTypes))
                    {
                        int previewSize = Mathf.Clamp(kTextureSize, c.width >> (c.mipmapCount - 1), c.width);

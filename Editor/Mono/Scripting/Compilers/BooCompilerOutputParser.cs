...
namespace UnityEditor.Scripting.Compilers
{
    class BooCompilerOutputParser : CompilerOutputParserBase
    {
        ...
        {
            var status = TryNormalizeCompilerStatus(match, "0019", sMissingMember, NormalizeMemberNotFoundError);
            if (status.code != NormalizedCompilerStatusCode.NotNormalized)
                return status;

            return TryNormalizeCompilerStatus(match, "0018", sUnknownTypeOrNamespace, NormalizeSimpleUnknownTypeOfNamespaceError);
        }
    }
}

...
namespace UnityEditor.Scripting.ScriptCompilation
{
    class EditorCompilation
    {
        ...
        public void DirtyAllNonCodeGenAssemblies(EditorScriptCompilationOptions additionalOptions)
        {
            ...
            var _ = CreateEditorScriptAssemblySettings(EditorScriptCompilationOptions.BuildingForEditor | additionalOptions);
        ...
        {
            CustomScriptAssembly customScriptAssembly;
            if (!filePathToCustomScriptAssemblies.TryGetValue(assemblyName, out customScriptAssembly))
            {
                DirtyAllScripts();
                return;
            }
            ...
            dirtyTargetAssemblies.Add(customTargetAssembly);
        }
        ...
        static void UpdateCustomTargetAssembliesResponseFileData(CustomScriptAssembly[] customScriptAssemblies, ResponseFileProvider responseFileProvider)
        {
            foreach (var assembly in customScriptAssemblies)
            {
                ...
                {
                    var responseFileContent = ScriptCompilerBase.GetResponseFileContent(Directory.GetParent(Application.dataPath).FullName, rspFile);
                    var compilerOptions = ScriptCompilerBase.GetCompilerOptions(responseFileContent);
                    assembly.ResponseFileDefines = ScriptCompilerBase.GetDefines(compilerOptions).ToArray();
                }
            }
        }
        ...
        public CompileStatus CompileScripts(EditorScriptCompilationOptions options, BuildTargetGroup platformGroup, BuildTarget platform, CompilationTaskOptions compilationTaskOptions = CompilationTaskOptions.StopOnFirstError)
        {
            var scriptAssemblySettings = CreateScriptAssemblySettings(platformGroup, platform, options);
        ...
        internal bool CompileCustomNonCodegenScriptAssemblies(EditorScriptCompilationOptions options, BuildTargetGroup platformGroup, BuildTarget platform)
        {
            DeleteUnusedAssemblies();
            DirtyAllNonCodeGenAssemblies(options);
            return CompileScripts(options, platformGroup, platform, CompilationTaskOptions.None) == CompileStatus.CompilationStarted;
        }
        ...
        static bool ShouldAddTargetAssemblyToList(TargetAssembly targetAssembly, ScriptAssemblySettings scriptAssemblySettings)
        {

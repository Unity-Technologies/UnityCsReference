...
class RestrictedApisValidator
{
    ...
    static RestrictedApiUsage[] GetRestrictedApiUsageForAssembly(string assemblyPath, RestrictedApisConfig restrictedApisConfig, bool logReferences)
    {
        ...
        var symbolReaderProvider = File.Exists(Path.ChangeExtension(assemblyPath, ".pdb")) ? new PortablePdbReaderProvider() : null;
        using var assembly = AssemblyDefinition.ReadAssembly(assemblyPath, new ReaderParameters { SymbolReaderProvider = symbolReaderProvider });

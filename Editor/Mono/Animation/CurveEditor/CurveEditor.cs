...
#pragma warning restore UA2001
        ...
        private void RecalculateSelectionWithCurvesBounds()
        {
            ...
            if (hasSelection)
            {
                var selected = new List<CurveSelection>(selectedCurves);
                selected.Sort();

                int currentCurveId = selected[0].curveID;
...
#pragma warning restore UA2001
        ...
        void StartEditingSelectedPoints(Vector2 fieldPosition)
        {
            ...
            if (selectedCurves.TrueForAll(x => GetKeyframeFromSelection(x).time == keyframe.time))
                m_NewTime = keyframe.time;
            if (selectedCurves.TrueForAll(x => GetKeyframeFromSelection(x).value == keyframe.value))
            ...
            string label)
        {
            ...
            bool sameValues = selectedCurves.TrueForAll(x => memberGetter(x) == firstSelectedValue);
            if (!sameValues)
        ...
        WrapMode WrapModeIconPopup(Keyframe key, WrapMode oldWrap, float hOffset)
        {
            ...
            Array enumValues = Enum.GetValues(typeof(WrapModeFixedCurve));
            var stringNames = Array.ConvertAll(Enum.GetNames(typeof(WrapModeFixedCurve)), ObjectNames.NicifyVariableName);
            ...
            return (WrapMode)enumValues.GetValue(selectedPopupIndex);
        }

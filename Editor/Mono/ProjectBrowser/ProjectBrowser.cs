...
namespace UnityEditor
{
    ...
    internal class ProjectBrowser : EditorWindow, IHasCustomMenu, ISearchableContainer, IFramableContainer
    {
        ...
        void FolderTreeSelectionCallback(EntityId[] selectedTreeViewInstanceIDs)
        {
            ...
            EntityId firstTreeViewInstanceID = EntityId.None;
            if (selectedTreeViewInstanceIDs.Length > 0)
                firstTreeViewInstanceID = selectedTreeViewInstanceIDs[0];

            bool folderWasSelected = false;
            if (firstTreeViewInstanceID != EntityId.None)
            {
        ...
        bool ValidateFilter(EntityId savedFilterID, SearchFilter filter)
        {
            if (filter == null)
                return false;
        ...
        {
            EntityId m_SavedFilterEntityId;

            static internal void Show(EntityId savedFilterEntityId)
            {
                // Curve context menu
                GUIContent delete = EditorGUIUtility.TrTextContent("Delete");

                GenericMenu menu = new GenericMenu();
                menu.AddItem(delete, false, new SavedFiltersContextMenu(savedFilterEntityId).Delete);

                menu.ShowAsContext();
            }

            private SavedFiltersContextMenu(EntityId savedFilterEntityId)
            {
                m_SavedFilterEntityId = savedFilterEntityId;
            }

            private void Delete()
            {
                DeleteFilter(m_SavedFilterEntityId);
            }
        }

...
namespace UnityEditor
{
    ...
    internal class VulkanDeviceFilterListsEditor : Editor
    {
        ...
        {
            m_HardwareProfilesUsed =
                BuildTargetDiscovery.TryGetBuildTarget(EditorUserBuildSettings.activeBuildTarget, out var iBuildTarget) &&
                (iBuildTarget?.GraphicsPlatformProperties?.HardwareProfilesUsed ?? false);

            var root = new VisualElement();

            // Hardware profiles warning
            var hwWarning = new HelpBox(
                DeviceFilterUI.Styles.hardwareProfilesOverrideWarning,
                HelpBoxMessageType.Warning);
            hwWarning.style.display = m_HardwareProfilesUsed ? DisplayStyle.Flex : DisplayStyle.None;
            root.Add(hwWarning);
            ...
            // Disable foldout CONTENT when hardware profiles override
            allowFoldout.contentContainer.SetEnabled(!m_HardwareProfilesUsed);
            denyFoldout.contentContainer.SetEnabled(!m_HardwareProfilesUsed);
            gfxJobsFoldout.contentContainer.SetEnabled(!m_HardwareProfilesUsed);
